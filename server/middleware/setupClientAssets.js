...
const getManifest = async () => {
  const manifest = await import(
    /* webpackChunkName: "client-asset-manifest" */ '../config/client-asset-manifest.json'
  )
  return manifest
}
...
const getLinkTag = ({ assetBaseUrl, manifest, assets }) => {
  if (!manifest) {
    ...
    console.log(
      `setupClientAssets: unable to find a manifest file at the path 'server/client/client-asset-manifest.json'`,
    )
    return ''
  }
  return assets
    ...
    .filter(
      (asset) => manifest?.assets?.[asset] && manifest?.assets?.[asset]?.path,
    )
...
const getScriptTags = ({ assetBaseUrl, manifest, assets }) => {
  if (!manifest) {
    // eslint-disable-next-line no-console
    console.log(
      `setupClientAssets: unable to find a manifest file at the path 'server/client/client-asset-manifest.json', will use ${assetBaseUrl}bundle.js`,
    )
